...
function ExportCustomerModal(props) {
  ...
  const { contactPoints } = useSelector((state) => state.customerManagement);
  ...
  const statusOptions = [
    <Option value={"1"} key={"active"}>
      Active
    </Option>,
    <Option value={"0"} key={"inactive"}>
      Inactive
    </Option>,
  ];
  ...
  return (
    <Modal
      ...
      width={"70vw"}
    >
      <Form form={form} id="exportForm" onFinish={onFinish} layout="vertical">
        <Form.Item>
          <Form.Item
            label="Customer name"
            style={{
